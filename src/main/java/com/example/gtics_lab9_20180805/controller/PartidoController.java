...
import com.example.gtics_lab9_20180805.entity.*;
import com.example.gtics_lab9_20180805.repository.HistorialPartidoRepository;
import com.example.gtics_lab9_20180805.repository.ParticipantePartidoRepository;
import com.example.gtics_lab9_20180805.repository.ParticipanteRepository;
import com.example.gtics_lab9_20180805.repository.PartidoRepository;
...
public class PartidoController {
    ...
    public ResponseEntity<HashMap<String, Object>> guardarPartido(
        ...
        try {
            ...
            for (Participante pA : PequipoA) {
                participantePartidoRepository.crearParticipantesPartido(partido.getIdPartido(),pA.getIdParticipante(),fechaComoDate);
            }

            for (Participante pB : PequipoB) {
                participantePartidoRepository.crearParticipantesPartido(partido.getIdPartido(),pB.getIdParticipante(),fechaComoDate);
            }
    ...
    public ResponseEntity<List<Participantepartido>> listaParticipantesPartido(@RequestParam(name = "idequipo", required = false) String idStr) {
        try {
            if (idStr != null) {
    ...
    public ResponseEntity<List<Historialpartido>> listaHistorial(@RequestParam(name = "idequipo", required = false) String idStr) {
        ...
        try {
            if (idStr != null) {
                int idEquipo = Integer.parseInt(idStr);
                List<Historialpartido> historiales = historialPartidoRepository.buscarHistorialEquipo(idEquipo);
    ...
    public ResponseEntity<HashMap<String, String>> gestionExceptionD(HttpServletRequest request) {
        HashMap<String, String> responseMap4 = new HashMap<>();

        if (request.getMethod().equals("POST") || request.getMethod().equals("PUT")) {
            responseMap4.put("estado", "error");
            responseMap4.put("msg", "Se debe incluir partido");
        }
